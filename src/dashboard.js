...
async function render() {
  try {
    ...
    console.log(
      `    Total Volume: ${formatUSD(world.totalVolume)}  |  Total Swaps: ${world.totalSwaps}  |  Fees: ${formatUSD(world.totalFees)}`
    );
    console.log();
    ...
    if (leaderboard && leaderboard.length > 0) {
      ...
      leaderboard.forEach((entry, i) => {
        ...
        console.log(
          `    ${pad(rank, 6)}${pad(entry.name, 18)}${rpad(formatUSD(entry.volume), 14)}${rpad(entry.swaps, 8)}${rpad(entry.reputation, 6)}`
        );
      });
      console.log();
    }
    ...
    if (intents && intents.length > 0) {
      ...
      intents.slice(0, 8).forEach((intent) => {
        console.log(
          `    ${pad(intent.agent, 16)} gives ${intent.give.amount} ${pad(intent.give.token, 6)} wants ${intent.want.token}`
        );
      });
      console.log();
    }
